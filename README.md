...
## VPS一键四协议安装脚本
```
bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//sing-box/main/sing-box.sh)
```
## vps带端口变量运行示列
PORT=开放的端口 确保后面3个端口可用 CFIP为优选IP或优选域名，CFPORT为优选ip或优选域名对应的端口
```
PORT=你的端口 CFIP=www.visa.com.tw CFPORT=443 bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//sing-box/main/sing-box.sh)
```


## ssh综合工具箱一键脚本
```
curl -fsSL https://raw.githubusercontent.com/XyHK-HUC//ssh_tool/main/ssh_tool.sh -o ssh_tool.sh && chmod +x ssh_tool.sh && ./ssh_tool.sh
```
...
```
bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//sing-box/main/sb_serv00.sh)
```
...
```
VMESS_PORT=tcp端口 HY2_PORT=udp端口 TUIC_PORT=udp端口 bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//sing-box/main/sb_00.sh)
```

## Serv00|CT8一键三协议安装脚本vless-reality|hy2|tuic5 
```
bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//sing-box/test/sb_00.sh)
```
...
```
VLESS_PORT=TCP端口 ARGO_PORT=TCP端口 bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//sing-box/test/x00.sh)
```
...
或
```
PORT=UDP端口 bash <(curl -Ls https://github.com/XyHK-HUC//Sing-box/releases/download/00/2.sh)
```
...
或
```
PORT=UDP端口 bash <(curl -Ls https://github.com/XyHK-HUC//Sing-box/releases/download/00/tu.sh)
```
...
```
PORT=TCP端口 bash <(curl -Ls https://raw.githubusercontent.com/XyHK-HUC//scripts/master/containers-shell/00_vmess.sh)
```
